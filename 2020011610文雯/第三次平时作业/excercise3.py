...
def MCC(func):
    def MCC_wrapper(*args,**kwargs):
        ...
        if multi_add_sum==0:         #如果任意TP,FP,FN,FN四项，任意两项为零，那么设multiaddtsum为1
            multi_add_sum=1
    ...
    for item in range(0,num):
        for key,value in Kwargs["struct"].items():
            if key=="int":
                ...
                break
        result.append(tmp)
    print("final_random_result:",result)
    return result

result=structDataSampling(num=1000,**{"struct":{"int":{"datarange":(0,1)}}})
print(result)